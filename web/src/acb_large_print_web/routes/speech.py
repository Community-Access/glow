...
_DOC_PREPARE_NAME = "speech_prepare.json"
_DOC_ALLOWED_EXTENSIONS = (set(CONVERT_EXTENSIONS) - AUDIO_EXTENSIONS - {".zip"}) | {".txt", ".rst"}
# Files generated by the speech pipeline that must never be treated as the
# user's source document when scanning an upload session directory.
_INTERNAL_SPEECH_FILES = {
    _DOC_EXTRACT_NAME,
    _DOC_RENDERED_NAME,
    _DOC_META_NAME,
    _DOC_PREPARE_NAME,
...
def _load_prepared_summary(temp_dir: Path, token: str) -> dict | None:
    """Restore the persisted preparation summary for an upload session.

    The async preparation job (run_speech_prepare_job) writes a
    ``speech_prepare.json`` summary and redirects the user back to
    ``/speech/?token=...``. This re-reads that summary so the page can restore
    the estimate and reveal the preview/download actions instead of looking
    like a fresh, un-prepared upload (issue #84).

    Returns ``None`` unless the summary is present, valid, references this
    token, and the extracted text file still exists and is non-empty.
    """
    prepare_path = temp_dir / _DOC_PREPARE_NAME
    extract_path = temp_dir / _DOC_EXTRACT_NAME
    if not prepare_path.exists() or not extract_path.exists():
        return None
    try:
        if extract_path.stat().st_size == 0:
            return None
        summary = json.loads(prepare_path.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return None
    if not isinstance(summary, dict) or summary.get("token") != token:
        return None
    required = (
        "token",
        "filename",
...
def speech_form():
    ...
    if prefill_token:
        ...
        if temp_dir is not None:
            # Look for the user's source file, ignoring any files generated by
            # the speech pipeline itself (extracted text, rendered text, etc.).
            all_files = sorted(
                f
                for f in temp_dir.iterdir()
                if f.is_file() and f.name not in _INTERNAL_SPEECH_FILES
            )
            if all_files:
            ...
            prepared_summary = _load_prepared_summary(temp_dir, prefill_token)
        else:
